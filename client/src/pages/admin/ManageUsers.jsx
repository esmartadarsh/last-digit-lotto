...
export default function ManageUsers() {
  ...
  const handleAdjustBalance = async (userId, userEmail, currentBalance) => {
    ...
    try {
      ...
      const res = await axios.post(`${API_BASE_URL}/admin/users/${userId}/adjust-balance`, { amount }, {
        headers: { Authorization: `Bearer ${token}` }
      });
      toast.success(res.data.message, { id: 'adj' });
      fetchUsers();
    } catch (err) {
            ...
            {loading ? (
              ...
              <tr key={u.id} className="hover:bg-slate-800/50 transition-colors">
                ...
                <td className="px-6 py-4 text-right">
                    <button 
                       onClick={() => handleAdjustBalance(u.id, u.email, u.balance)} 
                       className="text-blue-400 hover:text-blue-300 font-medium px-3 py-1.5 rounded bg-blue-500/10 hover:bg-blue-500/20 transition-colors"
                    >
                        Adjust Balance + / -
                    </button>
                </td>
